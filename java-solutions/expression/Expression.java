...
public interface Expression extends ToMiniString {
    ...
    static ExpressionTester<?, ?> tester(final TestCounter counter) {
        ...
        //noinspection Convert2MethodRef,RedundantSuppression
        return new ExpressionTester<>(
                ...
                .basic("(10 / (3 / 2))", "10 / (3 / 2)", x -> 10, new Divide(c(10), new Divide(c(3), c(2))))
                .basic("(10 * (3 / 2))", "10 * (3 / 2)", x -> 10, new Multiply(c(10), new Divide(c(3), c(2))))
                ...
                .basic("(x / x)", "x / x", x -> 1, new Divide(vx, vx))

                .advanced("(2 + 1)", "2 + 1", x -> 2 + 1, new Add(c2, c1))
                .advanced("(x - 1)", "x - 1", x -> x - 1, new Subtract(vx, c1))
                .advanced("(1 * 2)", "1 * 2", x -> 2, new Multiply(c1, c2))
                .advanced("(x / 1)", "x / 1", x -> x, new Divide(vx, c1))
                .advanced("(1 + (2 + 1))", "1 + 2 + 1", x -> 1 + 2 + 1, new Add(c1, new Add(c2, c1)))
                .advanced("(x - (x - 1))", "x - (x - 1)", x -> x - (x - 1), new Subtract(vx, new Subtract(vx, c1)))
                .advanced("(2 * (x / 1))", "2 * (x / 1)", x -> 2 * (x), new Multiply(c2, new Divide(vx, c1)))
                .advanced("(2 / (x - 1))", "2 / (x - 1)", x -> 2 / (x - 1), new Divide(c2, new Subtract(vx, c1)))
                .advanced("((1 * 2) + x)", "1 * 2 + x", x -> 2 + x, new Add(new Multiply(c1, c2), vx))
                .advanced("((x - 1) - 2)", "x - 1 - 2", x -> x - 1 - 2, new Subtract(new Subtract(vx, c1), c2))
                .advanced("((x / 1) * 2)", "x / 1 * 2", x -> x * 2, new Multiply(new Divide(vx, c1), c2))
                .advanced("((2 + 1) / 1)", "(2 + 1) / 1", x -> (2 + 1), new Divide(new Add(c2, c1), c1))
                .advanced("(1 + (1 + (2 + 1)))", "1 + 1 + 2 + 1", x -> 1 + 1 + 2 + 1, new Add(c1, new Add(c1, new Add(c2, c1))))
                .advanced("(x - ((1 * 2) + x))", "x - (1 * 2 + x)", x -> x - (2 + x), new Subtract(vx, new Add(new Multiply(c1, c2), vx)))
                ...
                .advanced("((1 * 2) + (2 + 1))", "1 * 2 + 2 + 1", x -> 2 + 2 + 1, new Add(new Multiply(c1, c2), new Add(c2, c1)))
                .advanced("((2 + 1) - (2 + 1))", "2 + 1 - (2 + 1)", x -> 0, new Subtract(new Add(c2, c1), new Add(c2, c1)))
                .advanced("((x - 1) * (x / 1))", "(x - 1) * (x / 1)", x -> (x - 1) * (x), new Multiply(new Subtract(vx, c1), new Divide(vx, c1)))
                .advanced("((x - 1) / (1 * 2))", "(x - 1) / (1 * 2)", x -> (x - 1) / (2), new Divide(new Subtract(vx, c1), new Multiply(c1, c2)))
                .advanced("(((x - 1) - 2) + x)", "x - 1 - 2 + x", x -> x - 1 - 2 + x, new Add(new Subtract(new Subtract(vx, c1), c2), vx))
                .advanced("(((1 * 2) + x) - 1)", "1 * 2 + x - 1", x -> 2 + x - 1, new Subtract(new Add(new Multiply(c1, c2), vx), c1))
                .advanced("(((2 + 1) / 1) * x)", "(2 + 1) / 1 * x", x -> (2 + 1) * x, new Multiply(new Divide(new Add(c2, c1), c1), vx))
