 ...
 */
@FunctionalInterface
@SuppressWarnings({"ClassReferencesSubclass", "divzero"})
public interface DoubleExpression extends ToMiniString {
    ...
    static ExpressionTester<?, ?> tester(final TestCounter counter) {
        ...
        return new ExpressionTester<>(
                ...
                .basic("(x / x)", "x / x", x -> 1.0, new Divide(vx, vx))

                .advanced("(x + x)", "x + x", x -> x + x, new Add(vx, vx))
                .advanced("(x - x)", "x - x", x -> 0.0, new Subtract(vx, vx))
                ...
                .advanced("(x / (x - x))", "x / (x - x)", x -> x / (0.0), new Divide(vx, new Subtract(vx, vx)))
                .advanced("((x - x) + 1.1)", "x - x + 1.1", x -> 0.0 + 1.1, new Add(new Subtract(vx, vx), c(1.1)))
                ...
                .advanced("((x - x) / 2.1)", "(x - x) / 2.1", x -> (0.0) / 2.1, new Divide(new Subtract(vx, vx),
                        c(2.1)
                ))
                ...
                .advanced("(2.1 - (2.1 - (1.1 * x)))", "2.1 - (2.1 - 1.1 * x)", x -> 2.1 - (2.1 - 1.1 * x), new Subtract(
                        ...
                        c(1.1), vx))))
                .advanced("(1.1 * ((x - x) + 1.1))", "1.1 * (x - x + 1.1)", x -> 1.1 * (0.0 + 1.1), new Multiply(c(1.1), new Add(new Subtract(vx, vx),
                        c(1.1)
                )))
                ...
                .advanced("(((x - x) / 2.1) + 2.1)", "(x - x) / 2.1 + 2.1", x -> (0.0) / 2.1 + 2.1, new Add(new Divide(new Subtract(vx, vx),
                        c(2.1)
                ), c(2.1)))
                .advanced("((x / (x - x)) - 1.1)", "x / (x - x) - 1.1", x -> x / (0.0) - 1.1, new Subtract(new Divide(vx, new Subtract(vx, vx)),
                        c(1.1)
                ))
                .advanced("((2.1 - (1.1 * x)) * 1.1)", "(2.1 - 1.1 * x) * 1.1", x -> (2.1 - 1.1 * x) * 1.1, new Multiply(new Subtract(
                        c(2.1), new Multiply(
                        c(1.1), vx)), c(1.1)))
                .advanced("((x / (x - x)) / x)", "x / (x - x) / x", x -> x / (0.0) / x, new Divide(new Divide(vx, new Subtract(vx, vx)), vx))
                ...
                .advanced("(((x - x) / 2.3E12) + 2.3E12)", "(x - x) / 2.3E12 + 2.3E12", x -> (0.0) / 2.3E12 + 2.3E12, new Add(new Divide(new Subtract(vx, vx),
                        c(2.3E12)
                ), c(2.3E12)))
                .advanced("((x / (x - x)) - 1.1E10)", "x / (x - x) - 1.1E10", x -> x / (0.0) - 1.1E10, new Subtract(new Divide(vx, new Subtract(vx, vx)),
                        c(1.1E10)
                ))
